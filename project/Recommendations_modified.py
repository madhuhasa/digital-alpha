...
for j in itemListt:
    ...
    recoListt.append(recoList)    
    for i in range(len(recoListt)):
        Recommendations=[]
        for u in range(len(recoListt[i])):
            
            if (recoListt[i]['score'][u] > 0):
                Recommendations.append(recoListt[i]['item2'][u])
        print("\n\n Recommendations for",itemListt[i]," \n",Recommendations[:5])


pickle_out = open("Product_Recommendation.p","wb")
pickle.dump(recoListt,pickle_out)
pickle_out.close()
